...
The initial migration creates six tables and PostgreSQL constraints/triggers. The second migration, `202609170001_ProfileAndPlans`, adds `user_profile` (one row per account: SimBrief username, callsign, home base, units, workspace preference, time zone, avatar initials, last-seen stamp) and constrains `SubscriptionStatus` to `active`, `complimentary`, `canceled` or `expired`. `AccountsDbContextModelSnapshot` points at `ProfileAndPlansModel`, which composes the frozen `InitialAccountsModel`; the next migration should compose `ProfileAndPlansModel` the same way and re-point the snapshot. Never edit an earlier model builder, and do not use `EnsureCreated`: that bypasses the security constraints supplied by the migrations.

`complimentary` marks a level granted without payment during early access (`SetPersonalPlanAsync`, `SetAirlinePlanAsync`). It counts as a current subscription everywhere `active` does, including the one-Community-airline limit. Billing can later replace it with `active` plus an expiry without changing the contracts.
